...
To prepare data for this figure, fire_1 was created from fire and assigned to be a dataframe. Then, we dealt with the date column because the type of date was known as character. Therefore, we use the format function to assign them as day, month, and year, respectively, and assigned them in each new column by “mutate” function in Tidyverse. After that, the dataframe was grouped by month and day and was counted to determine the frequency of wildfires in each day. Finally, we create a new column using make_date function to store date data to be x-axis in our graph. 
After prepared the fire_1, GGPLOT2 was used to visualize by using the geom_line function. The frequency of wildfires in each day was average by 27, which is the number of years in our data. Therefore, the y-axis is the average frequency of wildfires and the x-axis is the date in one year. 
To summarize, over the past 26 years from 1992 to 2018, July is the month that wildfires occur more often than the other months. The reason is that this month is in summer, so high temperatures together with dry weather can easily ignite wildfires.
...
To prepare data to Figure 3, fire_3 was created from fire and assigned to be a dataframe. Then, the data were grouped by the “NWCG_CAUSE_CLASSIFICATION” column. After that, the data were summarized and counted by summarize function in Tidyverse. Finally, the data were shown in the pie chart format by GGPLOT2. The graph indicated human behavior has a large amount of impact on wildfire than other types of causes.

To prepare data to Figure 4, fire_4 was created from fire and assigned to be a dataframe and then was arranged in the same way as fire_3. Finally, the data were shown in the bar chart format and then the graph was flipped in the horizontal for better understanding.
After the graph was analyzed, human behavior such as arson, recreation and ceremony, and debris and open burning has a large impact on wildfires. However, several data cannot be identified for the cause of wildfires, which is shown as missing data/ unspecified/ undetermined
