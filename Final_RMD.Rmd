...
Title 1 
------------------------------------------
```{r average wildfire coucsant by Day of Year, fig.cap="Avercsage Number of US Wildfires by Day of Year. Observe the trend of wildfires count over Time",  out.width='105%', out.height='105%', echo=TRUE, warning=FALSE}
fires_date <- fires %>% 
  select(FIRE_YEAR, FIRE_SIZE) %>% 
  group_by(FIRE_YEAR) %>% 
  summarise(Total_Fires = n(), Burn_Size = sum(FIRE_SIZE))
fires_date %>% 
  ggplot(aes(x= FIRE_YEAR, y = Total_Fires)) + 
  geom_bar(stat = 'identity', fill = 'darkred') + 
  geom_smooth(method = 'lm') +
  scale_x_continuous(name = " Year", breaks = round(seq(min(fires_date$FIRE_YEAR), max(fires_date$FIRE_YEAR), by = 2),1)) +
...
Title 1 
------------------------------------------
```{r average wildfire coucsant caby Day of Year, fig.cap="Avercsage Number of US Wildfires by Day of Year. Observe the trend of wildfires count over Time",  out.width='106%', out.height='106%', echo=TRUE, warning=FALSE}
fires_date %>% 
  ggplot(aes(x= FIRE_YEAR, y = Burn_Size)) + 
  geom_line(size = 1, color = "darkred") + 
  geom_smooth(method = 'lm') +  
  scale_x_continuous(name = "Year", breaks = round(seq(min(fires_date$FIRE_YEAR), max(fires_date$FIRE_YEAR), by = 2),1)) +
  scale_y_continuous(name = "Acres", labels = scales::comma) + 
  labs(x = '', y = 'Number of Acres Burned', title = 'US Wildfires by Year')
```
